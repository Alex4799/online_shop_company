    ...
    <div class="row">
        <div class="card col-md-8 offset-md-2">
            <div class="card-body">
              ...
              <form action="{{route('user#productAdd')}}" method="post" enctype="multipart/form-data">
                @csrf
                ...
                <div class="row">
                    <div class="py-2 col-md-6">
                        ...
                        <div class="">
                            ...
                            <div class="py-2">
                                <input type="file" name="image[]" class="form-control" id="input_image" multiple required>
                                @error('image')
                                    <small class=" text-danger">{{$message}}</small>
                                @enderror
                            </div>
                        ...
                        <div class="py-2 ">
                            ...
                            <select name="category_id" id="" class="form-control" >
                                ...
                                @foreach ($category as $item)
                                    <option value="{{$item->id}}" @if (old('category_id')==$item->id) @selected(true) @endif>{{$item->name}}</option>
                                @endforeach
                            </select>
                            @error('category_id')
                            <small class=" text-danger">{{$message}}</small>
                        ...
                        <div class="py-2">
                            ...
                            <select name="brand_id" id="" class="form-control" >
                                ...
                                @foreach ($brand as $item)
                                    <option value="{{$item->id}}" @if (old('brand_id')==$item->id) @selected(true) @endif>{{$item->name}}</option>
                                @endforeach
                            </select>
                            @error('brand_id')
                            <small class=" text-danger">{{$message}}</small>
